...
import { View, StyleSheet, Platform, TouchableOpacity, useWindowDimensions, TextInput, Button, DatePickerIOS, DatePickerAndroid, Linking } from 'react-native'
...
const HistoryScreen = ({ navigation }) => {
    ...
    const initCondition = () => {
        ...
        setNPerPage(20);
        setCounts(0);
        setStartDate({ year: null, month: null, date: null, hour: null, minute: null, second: null });
        setEndDate({ year: null, month: null, date: null, hour: null, minute: null, second: null });
    }
    ...
    useEffect(() => {
        if ((deviceID != _deviceID) || !sampleType.has(_sampleType)) {
            ...
            askForHistory(
                ...
                { deviceID: _deviceID, page: 0, nPerPage, sampleType: new Set([_sampleType]) }
            );
        }
    ...
    const onDeviceChange = (value) => {
        ...
        askForHistory(
            ...
            { deviceID: _currentHistoryData[value].deviceID, page: 0, nPerPage, sampleType: new Set([_currentHistoryData[value].sampleType]) }
        );
    }
    ...
    async function askForHistory(handle, param) {
        if (param) {
            var { deviceID, page, nPerPage, dataType, startDate, endDate, type, sampleType } = param;
            if (type == "download") {
                if (sampleType.size == 1 && ([...sampleType][0] == "")) {
                    param = { deviceID, page, nPerPage, sampleType: [], dataType, startDate, endDate, type, filename: filename + yearMonth(startDate) + "~" + yearMonth(endDate) }
                } else {
                    param = { deviceID, page, nPerPage, sampleType: new Array(...sampleType), dataType, startDate, endDate, type, filename: filename + yearMonth(startDate) + "~" + yearMonth(endDate) }
                }
            } else {
                if (sampleType.size == 1 && ([...sampleType][0] == "")) {
                    param = { deviceID, page, nPerPage, sampleType: [], dataType, startDate, endDate: endDate };
                } else {
                    param = { deviceID, nPerPage, page, sampleType: new Array(...sampleType), ...(dataType ? { dataType } : {}), ...(startDate ? { startDate } : {}), ...(endDate ? { endDate } : {}) }
                }
            }
        } else {
            if (sampleType.size == 1 && ([...sampleType][0] == "")) {
                ...
                param = { deviceID, page, nPerPage, sampleType: new Array(...sampleType) };
            }
        }
        // console.log(param)
        const response = await trackerApi.post(`${uploadIP}/SmartDetectHistory`, param);
    ...
    const nextPageClick = () => {
        if ((page + 1) * nPerPage <= counts) {
            ...
            askForHistory(
                ...
                { deviceID, page: page + 1, nPerPage, sampleType, dataType, startDate, endDate }
            );
        }
    }
    const lastPageClick = () => {
    ...
    const jumpClick = () => {
        ...
        if (jump_page >= 1 && (jump_page - 1) * nPerPage < counts) {
            ...
            askForHistory(
                ...
                { deviceID, page: jump_page - 1, nPerPage, sampleType, dataType, startDate, endDate }
            );
        }
    }
    ...
    const yearMonth = time => time.getFullYear() + ("-" + (time.getMonth() + 1));
    const saveFile = () => {
        var { startDate, endDate } = getStart_EndTime();
        if ((!startDate) && (!endDate)) {
            return;
        ...
        var info = {
            "type": "download",
            deviceID, page, nPerPage, sampleType: new Array(...sampleType), dataType, startDate, endDate, filename: historyData.name + "(" + historyData.sampleType + ")" + yearMonth(startDate) + "~" + yearMonth(endDate)
        }
        Linking.openURL(uploadIP + "/SmartDetectHistory?" + new URLSearchParams(info).toString());
        setExportModalVisible(false);
    }

    const getTime = (dateTime) => {
        if (dateTime.year == null && dateTime.month == null && dateTime.date == null && dateTime.hour == null && dateTime.minute == null && dateTime.second == null) {
            return null
        } else {
    ...
    return (
        <SafeAreaView forceInset={{ top: 'always' }} style={{ flex: 1, marginTop: 20 }}>
            <CustomModal title={"其他条件"} modalVisible={conditionModalVisible} setModalVisible={setConditionModalVisible}
                footButton={
            ...
            >
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                    ...
                    <View style={{ flexDirection: "row", flex: 1, justifyContent: "space-between" }}>
                        <TouchableOpacity
                            style={{ ...styles.buttonInRow, ...(dataType == "normalData" ? { backgroundColor: "#15caf0" } : { backgroundColor: "white" }) }}
                            onPress={() => {
                                setDataType("normalData");
                            }}
                        >
                            <Text style={{ ...(dataType == "normalData" ? { color: 'black', fontWeight: "bold" } : { color: "#15caf0" }) }}>正常报表</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                            style={{ ...styles.buttonInRow, ...(dataType == "monthData" ? { backgroundColor: "#15caf0" } : { backgroundColor: "white" }) }}
                            onPress={() => {
                                setDataType("monthData");
                            }}
                        >
                            <Text style={{ ...(dataType == "monthData" ? { color: 'black', fontWeight: "bold" } : { color: "#15caf0" }) }}>月报表</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                            style={{ ...styles.buttonInRow, ...(dataType == "dailyData" ? { backgroundColor: "#15caf0" } : { backgroundColor: "white" }) }}
                            onPress={() => {
                                setDataType("dailyData");
                            }}
                        >
                            <Text style={{ ...(dataType == "dailyData" ? { color: 'black', fontWeight: "bold" } : { color: "#15caf0" }) }}>日报表</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                            style={{ ...styles.buttonInRow, ...(dataType == "hourData" ? { backgroundColor: "#15caf0" } : { backgroundColor: "white" }) }}
                            onPress={() => {
                                setDataType("hourData");
                            }}
                        >
                            <Text style={{ ...(dataType == "hourData" ? { color: 'black', fontWeight: "bold" } : { color: "#15caf0" }) }}>小时数据</Text>
                        </TouchableOpacity>
                    </View>
                </View>
                <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            <CustomModal title={"数据导出"}
                ...
                modalVisible={exportModalVisible} setModalVisible={setExportModalVisible}>
                <Text>
                    数据导出
                </Text>
            </CustomModal>
                ...
                <View style={styles.headerRight}>
                    <TouchableOpacity
                        style={{
                            ...
                            ... (startDate.year == null && startDate.month == null && startDate.date == null && startDate.hour == null && startDate.minute == null && startDate.second == null &&
                                endDate.year == null && endDate.month == null && endDate.date == null && endDate.hour == null && endDate.minute == null && endDate.second == null &&
                                dataType == "normalData") ? { backgroundColor: "#6b747d" } : { backgroundColor: "#316cf4" }
                        }}
            ...
            <HistoryTable needReset={needReset} headers={headers} dataList={dataList} tableConfig={{ tableHeight: window.height - (Platform.OS == 'ios' ? 253 : 199) }} />
            <View style={styles.footer}>
                ...
                />
                <TouchableOpacity
                    style={{ ...styles.button, backgroundColor: "#316cf4", marginRight: 10 }}
                    onPress={jumpClick}
                >
                ...
                <TouchableOpacity
                    ...
                    style={{ ...styles.button, backgroundColor: "white", borderColor: "#dee1e5", borderWidth: 1.2 }}
                >
                    {page >= 1 ? <Text style={{ color: "#316cf4" }}>上一页</Text> :
                ...
                <TouchableOpacity
                    ...
                    style={{ ...styles.button, backgroundColor: "white", borderColor: "#dee1e5", borderWidth: 1.2 }}
                >
                    {(page + 1) * nPerPage <= counts ? <Text style={{ color: "#316cf4" }}>下一页</Text> :
